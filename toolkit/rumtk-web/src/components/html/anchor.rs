...
pub fn anchor(_path_components: URLPath, params: URLParams, state: SharedAppState) -> HTMLResult {
    let id = rumtk_web_get_text_item!(params, PARAMS_ID, DEFAULT_TEXT_ITEM);
    let css_class = rumtk_web_get_text_item!(params, PARAMS_CSS_CLASS, DEFAULT_TEXT_ITEM);

    let custom_css_enabled = rumtk_web_get_config!(state).flags.custom_css;

    rumtk_web_render_template!(Anchor {
        id,
        css_class,
        custom_css_enabled
    })
...
pub fn a(_path_components: URLPath, params: URLParams, state: SharedAppState) -> HTMLResult {
