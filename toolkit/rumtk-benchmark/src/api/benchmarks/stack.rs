...
use rumtk_web::{rumtk_web_get_job_manager, rumtk_web_get_pipelines, rumtk_web_render_component, rumtk_web_render_page_contents, rumtk_web_render_template};
...
async fn basic_processor(form: FormData, state: SharedAppState) -> JobResult {
    match form.form.get("basic_choice") {
        Some(pipeline_name) => {
            let pipeline = rumtk_web_get_pipelines!(state).get_pipeline("basic", pipeline_name);
